...
class network(object):
  """ 具体网络在setup中重写,数据写在__init__"""
  def __init__(self, inputs, trainable=True):
  # inputs format: {'data':tf.placeholder, 'label':tf.placeholder}
    self.inputs = []
  ...
  @layer
  def conv(self, input, k_h, k_w, c_o, s_h, s_w, name, weight_decay, biased=True,relu=True, padding=DEFAULT_PADDING, trainable=True):
  ...
  @layer
  def upconv(self, input, shape, c_o, ksize=4, stride = 2, name = 'upconv',weight_decay, biased=False, relu=True, padding=DEFAULT_PADDING, trainable=True):
  """ up-conv"""
  self.validate_padding(padding)
  ...
  def spatial_reshape_layer(self, input, d, name):
    ...
    return tf.reshape( input,
                     [ input_shape[0], input_shape[1], -1, int(d)]
                      )
 ...
 @layer
  def fc(self, input, num_out, name, weight_decay, relu=True, trainable=True):
    with tf.variable_scope(name) as scope:
  ...
  @layer
  def pva_negation_block_v2(self, input, k_h, k_w, c_o, s_h, s_w, c_in, name, biased=True, padding=DEFAULT_PADDING, trainable=True,scale = True, negation = True):
  ...
  def pva_inception_res_stack(self, input, c_in, name, block_start = False, type = 'a'):
    ...
    with tf.variable_scope(name) as scope:
      if block_start:
        ...
        proj = self.conv._original(self, input, 1, 1, c_out, 2, 2, name='proj', biased=True, relu=False)
      else:
